...
BEGIN
    ...
    DECLARE linIdPeriodoInformarAnterior                INTEGER;
    DECLARE ctiCodigoClasifPersonaAfiliado              TINYINT;
    DECLARE ctiCodigoClasifPersonaPensionado            TINYINT;
    ...
    SET ctiCodigoClasifPersonaAfiliado              = 1;
    SET ctiCodigoClasifPersonaPensionado            = 2;
    ...
    SET linIdPeriodoInformarAnterior                = DMGestion.obtenerIdDimPeriodoInformarPorFecha(ldtFechaPeriodoCotizacion);
    ...
    IF (codigoError = cstCodigoErrorCero) THEN
        ...
        CREATE TABLE #FctComportamientoPago  (
            ...
            indTopeImponible3m              CHAR(2)         NULL,
            indTopeImponible6m              CHAR(2)         NULL,
            indTopeImponible12m             CHAR(2)         NULL,
            indStockTopeImponible6M         CHAR(2)         NULL,
            indStockTopeImponible12M        CHAR(2)         NULL,
            mesesDisponibles                BIGINT          NULL DEFAULT 0,
            periodosTope                    BIGINT          NULL DEFAULT 0,
            densidadCotizacionHistorica     NUMERIC(5,2)    NULL,
            densidadTopehabitat             NUMERIC(5,2)    NULL
        );
        ...
        --calcular mesesDisponibles Densidad cotizaciones y Tope
        SELECT
            dimper.rut,
            dimcla.codigo,
            dimcla.nombre,
            fct.fechaAfiliacionSistema,
            DATEDIFF(MONTH, fct.fechaAfiliacionSistema, ldtFechaPeriodoInformado) AS mesesDisponibles           
        INTO #AfiliacionSistemaDensidad
        FROM DMGestion.FctlsInformacionAfiliadoCliente fct
        INNER JOIN DMGestion.DimPersona dimper
            ON fct.idPersona = dimper.id
        INNER JOIN DMGestion.DimSubClasificacionPersona dimsubcla
            ON fct.idSubClasificacionPersona = dimsubcla.id
        INNER JOIN DMGestion.DimClasificacionPersona dimcla
            ON dimsubcla.idClasificacionPersona = dimcla.id
        WHERE fct.idPeriodoInformado = linIdPeriodoInformar
        AND dimcla.codigo IN (ctiCodigoClasifPersonaAfiliado, ctiCodigoClasifPersonaPensionado);
       ...
       --Se obtiene la cantidad de registros que tiene una persona, para el máximo periodo de cotización
        ...
        INTO #UniversoMesesCotizadosCCICO
        FROM DMGestion.UniversoMovimientosCotizacionTMP um
        ...
        SELECT um.id_mae_persona,
            COUNT(DISTINCT um.per_cot) numeroMesesCotizadosCCICO  --totalMesesCotizadosCCICO (10)
        INTO #UniversoCCICOAntesFechaAfiliacion
        FROM DMGestion.UniversoMovimientosCotizacionTMP um
        WHERE um.codigoTipoProducto = cinCicco --Producto CCICO
        ...
        INTO #UniversoNumeroMesesCotizados
        FROM DMGestion.UniversoMovimientosCotizacionTMP um
        ...
        SELECT um.id_mae_persona,
            COUNT(DISTINCT um.per_cot) numeroMesesCotizados  --totalMesesCotizados (11)
        INTO #UniversoCCICOyCCIAVAntesFechaAfiliacion
        FROM DMGestion.UniversoMovimientosCotizacionTMP um
        WHERE um.codigoTipoProducto IN (cinCicco, cinCciav) --Producto CCICO y CCIAV
       ...
       FROM #FctComportamientoPago a
        ...
        /********** NUEVOS INDUCADORES PARA MARCAR LOS TOPE CON 3 6 Y 12 MESES CONSECUTIVOS **********/

        SELECT rut INTO #topeImponibleMes
        FROM #FctComportamientoPago
        WHERE indTopeImponible = cchS;
        ...
        WHERE indTopeImponible = cchS
            AND idPeriodoInformado < linIdPeriodoInformar
            AND idPeriodoInformado >= (linIdPeriodoInformar-cinDos)
        GROUP BY dp.rut
        HAVING COUNT()= cinDos;
        ...
        WHERE indTopeImponible = cchS
            AND idPeriodoInformado < linIdPeriodoInformar
            AND idPeriodoInformado >= (linIdPeriodoInformar-cinCinco)
        GROUP BY dp.rut
        HAVING COUNT()= cinCinco;

        SELECT dp.rut INTO #universo12
        FROM DMgestion.fctComportamientoPago fcp
            INNER JOIN DMGestion.DimPersona dp ON dp.id = fcp.idPersona
            INNER JOIN DMGestion.DimPeriodoInformado dpi ON dpi.id = fcp.idPeriodoInformado
            INNER JOIN #topeImponibleMes tpi ON tpi.rut = dp.rut
        WHERE  indTopeImponible = cchS
            AND idPeriodoInformado < linIdPeriodoInformar
            AND idPeriodoInformado >= (linIdPeriodoInformar-cinOnce)
        GROUP BY dp.rut
        HAVING COUNT()=cinOnce;


        UPDATE #FctComportamientoPago
        SET indTopeImponible3m = CASE WHEN b.rut IS NULL THEN cchNo ELSE cchSi END
        FROM #FctComportamientoPago a
        LEFT OUTER JOIN #universo3 B ON a.rut = b.rut;

        UPDATE #FctComportamientoPago
        SET indTopeImponible6m = CASE WHEN b.rut IS NULL THEN cchNo ELSE cchSi END
        FROM #FctComportamientoPago a
        LEFT OUTER JOIN #universo6 B ON a.rut = b.rut;

        UPDATE #FctComportamientoPago
        SET indTopeImponible12m = CASE WHEN b.rut IS NULL THEN cchNo ELSE cchSi END
        FROM #FctComportamientoPago a
        LEFT OUTER JOIN #universo12 B ON a.rut = b.rut;


        ---INDICADORES DE STOCK DE 6 Y 12 MESES

        SELECT DISTINCT dp.rut INTO #stock6
        FROM DMgestion.FctComportamientoPago fcp
        INNER JOIN DMGestion.DimPersona dp ON dp.id = fcp.idPersona
        WHERE idPeriodoInformado >= linIdPeriodoInformar-cinCinco
        AND indTopeImponible3m = cchSi;


        SELECT DISTINCT dp.rut INTO #stock12
        FROM DMgestion.FctComportamientoPago fcp
        INNER JOIN DMGestion.DimPersona dp ON dp.id = fcp.idPersona
        WHERE idPeriodoInformado >= linIdPeriodoInformar-cinOnce
        AND indTopeImponible3m = cchSi;


        UPDATE #FctComportamientoPago
        SET indStockTopeImponible6M = CASE WHEN b.rut IS NULL THEN cchNo ELSE cchSi END
        FROM #FctComportamientoPago a
        LEFT OUTER JOIN #stock6 B ON b.rut = a.rut;

        UPDATE #FctComportamientoPago
        SET indStockTopeImponible12M = CASE WHEN b.rut IS NULL THEN cchNo ELSE cchSi END
        FROM #FctComportamientoPago a
        LEFT OUTER JOIN #stock12 B ON b.rut = a.rut;
        ...
        --calculo densidad densidadCotizacionHistorica
        SELECT 
            a.rut,
            a.totalMesesCotizados, 
            b.mesesDisponibles,
            ISNULL(CONVERT(NUMERIC(5,2), ROUND((CONVERT(NUMERIC(17,10), a.totalMesesCotizados) / CONVERT(NUMERIC(17,10), b.mesesDisponibles)) * 100, 2)), 0.00) AS densidadCotizacionHistorica
        INTO #UniversoDensidadHistorica
        FROM #FctComportamientoPago a 
        LEFT JOIN #AfiliacionSistemaDensidad b
            ON a.rut = b.rut;

        UPDATE #FctComportamientoPago SET 
            a.mesesDisponibles = b.mesesDisponibles,
            a.densidadCotizacionHistorica = b.densidadCotizacionHistorica
        FROM #FctComportamientoPago a
        JOIN #UniversoDensidadHistorica b
            ON a.rut = b.rut;

        --calculo densidad densidadTopeHabitat
        SELECT
            u.rut,
            COUNT(DISTINCT u.idPeriodoInformado) AS periodosTope
        INTO #PeriodosTopeDensidad
        FROM
        (
        SELECT
            fct.idPeriodoInformado,
            dimper.rut,
            fct.indTopeImponible
        FROM DMGestion.FctComportamientoPago fct
        INNER JOIN DMGestion.DimPersona dimper
            ON fct.idPersona = dimper.id
        WHERE fct.idPeriodoInformado < linIdPeriodoInformar
        AND fct.indTopeImponible = cchS
        ...
        SELECT 
            ...
            a.periodosTope,
            b.mesesDisponibles,
            ISNULL(CONVERT(NUMERIC(5,2), ROUND((CONVERT(NUMERIC(17,10), a.periodosTope) / CONVERT(NUMERIC(17,10), b.mesesDisponibles)) * 100, 2)), 0.00) AS densidadTopeHabitat
        INTO #UniversoDensidadTope
        FROM #PeriodosTopeDensidad a
        LEFT JOIN #AfiliacionSistemaDensidad b
            ON a.rut = b.rut;
        
        UPDATE #FctComportamientoPago SET 
            a.periodosTope = b.periodosTope,
            a.densidadTopehabitat = b.densidadTopeHabitat
        FROM #FctComportamientoPago a
        JOIN #UniversoDensidadTope b
            ON a.rut = b.rut;

        --regulariza registros en null par las variables mesesDisponibles, periodosTope, densidadCotizacionHistorica, densidadTopeHabitat
        UPDATE #FctComportamientoPago SET
            mesesDisponibles = 0
        WHERE mesesDisponibles IS NULL;

        UPDATE #FctComportamientoPago SET
            periodosTope = 0
        WHERE periodosTope IS NULL;

        UPDATE #FctComportamientoPago SET
            densidadCotizacionHistorica = 0.00
        WHERE densidadCotizacionHistorica IS NULL;

        UPDATE #FctComportamientoPago SET
            densidadTopeHabitat = 0.00
        WHERE densidadTopeHabitat IS NULL;
        ...
        SELECT a.idPersona,
            ...
            a.indTopeImponible3m,
            a.indTopeImponible6m,
            a.indTopeImponible12m,
            a.indStockTopeImponible6M,
            a.indStockTopeImponible12M,
            a.mesesDisponibles,
            a.periodosTope,
            a.densidadCotizacionHistorica,
            a.densidadTopehabitat
        ...
        -------------------------------------------------------------------------------------------
         INSERT INTO DMGestion.FctComportamientoPago(
            ...
            indTopeImponible6m,
            indTopeImponible12m,
            indStockTopeImponible6M,
            indStockTopeImponible12M,
            mesesDisponibles,
            periodosTope,
            densidadCotizacionHistorica,
            densidadTopehabitat
        )
