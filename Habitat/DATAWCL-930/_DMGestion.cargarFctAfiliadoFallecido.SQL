...
BEGIN
    ...
    **/
    ...
    IF (codigoError = cstCodigoErrorCero) THEN
        ...
        WITH RankedSolicPen AS (
            SELECT
                ...
                RANK() OVER (PARTITION BY numcue ORDER BY fecsol DESC) as rn -- Usar ROW_NUMBER si solo necesitas una fila arbitraria, o RANK si necesitas manejar empates.
        ...
        SELECT u.id_mae_persona,
            (CASE
                ...
                ELSE 
                    CONVERT(NUMERIC(20, 3), 0) --cero
             END) ingresoBase,
            CONVERT(NUMERIC(20, 3), 0) ingresoBaseUF,                   --10-03-2014 CGI (-)
            (CASE
                WHEN (u.tipoben = 8) THEN 
                    CONVERT(NUMERIC(20, 2), ISNULL(u.promedio_rentas_120meses, 0))
                ELSE CONVERT(NUMERIC(20, 2), 0) --cero
             END) promedioRentas,
            CONVERT(NUMERIC(20, 2), 0) promedioRentasUF,
            ISNULL(u.nro_meses_con_renta, 0) numeroRemuneraciones,
            CONVERT(DATE, DATEADD(MONTH, -1, fecsol)) fechaAnterior,
            CONVERT(DATE, NULL) ultimoDiaHabilMesAnterior,
            CONVERT(NUMERIC(10, 2), 0.0) valorUF
        ...
        FROM (SELECT MAX(ant_fec_emision) ant_fec_emision,ant_numrut 
              ...
              WHERE ant_tipo_pension = 'SO' 
              and ant_cambio_modalidad = 'N'
       ...
       INTO #FechaPrimerPagoPensionDefRV
        ...
        SELECT u.codigoSituacionAfiliado,
            ...
            (CASE
                ...
                ELSE 
                    CASE
                        WHEN ((u.IND_COBERTURA_SEG_INVALIDEZ='S') OR (u.cod_ciaseg > 0)) THEN
                            CASE 
                                WHEN (u.PORCENTAJE_COBERTURA_SEG IN (35, 50)) THEN 
                                    CONVERT(CHAR(2), '02') --parcial
                                WHEN (u.PORCENTAJE_COBERTURA_SEG = 70) THEN 
            ...
            CONVERT(CHAR(1), 'N') indIngresoSCOMP,
        ...
        SELECT CONVERT(BIGINT, NULL) numeroFila,
            ...
            --CASE WHEN u.fechaPrimerPagoPenDef <= ldtHastaFechaPeriodoInformar THEN u.fechaPrimerPagoPenDef ELSE NULL END  fechaPrimerPagoPension,
        ...
        /*SELECT * INTO  DMGestion.UniversoRegistroTMP
        ...
        FROM #UniversoRegistro u
            ...
            AND fpp.fecsol = u.fecsol
            AND fpp.idPersona = u.idPersona)
        WHERE u.codigoModalidadPension IN ('03', '04', '05', '06', '08', '09')
        AND u.fechaPrimerFichaCalculoRP IS NULL;
        ...
        FROM #UniversoRegistro u
            JOIN #UniversoPensionadoTMP m ON (m.tipoben = u.tipoben
        ...
        FROM #UniversoRegistro a
           JOIN #fechaSolicitudHerenciaTMP b ON (a.numcue = b.numcue);
        ...
        FROM DDS.SLB_MONTOBEN a
            ...
            AND a.tipoben = b.tipoben)
        WHERE a.tipoben = 8
        AND a.modalidad IN (1,5) 
        AND a.cod_financiamiento = 2
        AND a.tipo_pago = 2
        ...
        WHERE codigoModalidadPension NOT IN ('01', '02', '03', '04', '05', '06', '07', '08', '09');

        UPDATE #UniversoRegistro SET
            fechaCertificadoSaldo = NULL
        WHERE codigoModalidadPension IN ('08', '09')
        AND indIngresoSCOMP = 'N';
        ...
        /*SELECT * INTO DMGestion.UniversoRegistro
        FROM #UniversoRegistro;
        ...
        UPDATE #UniversoRegistro
           SET estadoSolicitud = 0--NO aplica 
        WHERE ISNULL(fechaPrimerPagoPension,cdtMaximaFechaVigencia) = cdtMaximaFechaVigencia
           AND idModalidadPension = 0
           AND estadoSolicitud = 0;
       
        UPDATE #UniversoRegistro
           SET estadoSolicitud = 4--APROBADA solo si es mayor que la fecha de corte y tiene fecha de primer pago del periodo
        WHERE ISNULL(fechaPrimerPagoPension,cdtMaximaFechaVigencia) <> cdtMaximaFechaVigencia
           AND idModalidadPension > 0
        ...
        UPDATE #UniversoRegistro
           SET estadoSolicitud = 4--APROBADA sin importar la fecha de primer pago anterior a la fecha de corte
        WHERE idModalidadPension > 0
           AND fechaSolPensionSob < cdtFecCorteNoPerfect
           AND estadoSolicitud = 0;
           
        UPDATE #UniversoRegistro
           SET estadoSolicitud = 104--APROBADA solo si es mayor que la fecha de corte y tiene fecha de primer pago del periodo
        WHERE ISNULL(fechaPrimerPagoPension,cdtMaximaFechaVigencia) = cdtMaximaFechaVigencia
           AND idModalidadPension > 0
           AND fechaSolPensionSob >= cdtFecCorteNoPerfect
           AND estadoSolicitud = 0;
    ...
    END IF;
 MESSAGE 'codigoError: '||codigoError TO CLIENT;
-------------------------------------------------------------------------------------------     
